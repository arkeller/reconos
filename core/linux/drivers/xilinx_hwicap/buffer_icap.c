...
{
	printk(KERN_ERR "buffer_icap_get_status\n");
		...
		u32 offset, u32 count)
{
	...
	if (buffer_icap_busy(base_address)) {
		printk(KERN_ERR "device_read: Busy\n");
		return -EBUSY;
	}

	if ((offset + count) > XHI_MAX_BUFFER_INTS){
		printk(KERN_ERR "device_read: offset + count to big\n");
		return -EINVAL;
	}
	...
	while (buffer_icap_busy(base_address)) {
		...
		if (retries > XHI_MAX_RETRIES){
			printk(KERN_ERR "device_read: waited too long for hw, giving up\n");
			return -EIO;
		}
	}
	return 0;
			     ...
			     u32 size)
{
	...
	u32 i;
	void __iomem *base_address = drvdata->base_address;
	printk(KERN_ERR "base address: %x\n", drvdata->base_address);
