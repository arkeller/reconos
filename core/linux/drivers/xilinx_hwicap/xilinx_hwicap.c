...
#define HWICAP_BASE 0x51000000 
#define HWICAP_HIGH 0x510FFFFF

static struct resource hwicap0[] = {
	{
		.start = HWICAP_BASE,
		.end = HWICAP_HIGH,
...
static struct platform_device hwicap_device = {
	.name		= "icap",
	.id		= 0,
	.resource = hwicap0,
	.dev = {
		.platform_data = "virtex2p",
		},
	.num_resources = ARRAY_SIZE(hwicap0),
...
static int hwicap_command_desync(struct hwicap_drvdata *drvdata)
{
	...
	int status1 = 0;
	int status2 = 0;
	printk(KERN_ERR "hwicap_cpmmand_desync\n");
	/*
	 ...
	 */
	status1 =  drvdata->config->set_configuration(drvdata,
			&buffer[0], index);
	status2 = drvdata->config->get_status(drvdata);
        printk(KERN_ERR "hwicap command desync status is %i\n", status2);
	return status1;
}
		...
		u32 reg, u32 *reg_data)
{
	...
	u32 buffer[6];
	u32 index = 0;
	printk(KERN_ERR "hwicap_get_configuration_register\n");
	/*
	 * Create the data to be written to the ICAP.
	 */
	...
	if ((status & XHI_SR_DALIGN_MASK) != XHI_SR_DALIGN_MASK){
		printk(KERN_ERR "get_config_error_1 status is %i\n", status);
		return -EIO;
	}
	...
	status = drvdata->config->set_configuration(drvdata,
			&buffer[0], index);
	if (status){
		printk(KERN_ERR "get_config_error_2\n");
		return status;
	}
	...
	if (status){
		printk(KERN_ERR "get_config_error_3\n");
		return status;
	}
	return 0;
}
...
static int hwicap_initialize_hwicap(struct hwicap_drvdata *drvdata)
{
	int status;
	u32 idcode = 0;
	printk(KERN_ERR "---> hwicap initializing\n");
	...
	printk(KERN_ERR "---> hwicap reading idcode\n");
	status = hwicap_get_configuration_register(
			drvdata, drvdata->config_regs->IDCODE, &idcode);
	dev_dbg(drvdata->dev, "IDCODE = %x\n", idcode);
	printk(KERN_ERR "IDCODE = %x\n", idcode);
	if (status){
		printk(KERN_ERR "hwicap_get_configuration_register returned a value, we exit\n");
		return status;
	}
	dev_dbg(drvdata->dev, "Desync...\n");
	printk("hwicap desync...\n");
	status = hwicap_command_desync(drvdata);
	if (status)
		return status;
	printk(KERN_ERR "---> hwicap initializing success\n");
	return 0;
}
		...
		size_t count, loff_t *ppos)
{
	...
	ssize_t len;
	ssize_t status;
	printk(KERN_ERR "hwicap_write called\n");
	status = mutex_lock_interruptible(&drvdata->sem);
	if (status)
		return status;
...
static int hwicap_open(struct inode *inode, struct file *file)
{
	...
	int status;
	printk(KERN_ERR "hwicap open called\n");
	lock_kernel();
	...
	if (status) {
		...
		printk(KERN_ERR "couldn't initialize hwicap\n");
		goto error;
	}
	...
	drvdata->is_open = 1;
	printk(KERN_ERR "hwicap open success\n");
...
static int hwicap_release(struct inode *inode, struct file *file)
{
	...
	int i;
	int status = 0;
	printk(KERN_ERR "hwicap_release called\n");
		...
		const struct config_registers *config_regs)
{
	...
	printk(KERN_ERR "hwicap setup success");
	return 0;		/* success */
...
{
	struct hwicap_drvdata *drvdata;
	printk(KERN_ERR "hwicap_remove called\n");
...
static int __devinit hwicap_drv_probe(struct platform_device *pdev)
{
	...
	const char *family;
	printk(KERN_ERR "hwicap_drv_probe started\n");
	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (!res)
	{
	
		printk(KERN_ERR "hwicap - couldn't finde hw\n");
		return -ENODEV;
	}
	...
	if (family != NULL)
		printk(KERN_ERR "family = NULL\n");
	if (family) {
		if (!strcmp(family, "virtex2p")) {
			printk(KERN_ERR "using virtex2pro\n");
	...
	printk(KERN_ERR "--> hwicap calling setup\n");
...
{
	printk(KERN_ERR "hwicap_drv_remove called\n");
...
hwicap_of_probe(struct of_device *op, const struct of_device_id *match)
{
	...
	const struct config_registers *regs;
	printk(KERN_ERR "hwicap_of_probe called\n");
...
{
	printk(KERN_ERR "hwicap_of_remove called\n");
...
{
	printk(KERN_ERR "hwicap_hwicap_of_register called\n");
...
{
	printk(KERN_ERR "hwicap_of_unregister called\n");
...
static int __init hwicap_module_init(void)
{
	dev_t devt;
	int retval;
	printk(KERN_ERR "---> hwicap module init\n");
	...
	if (retval < 0)
		return retval;
	printk(KERN_ERR "hwicap added device\n");
	...
	if (retval)
		goto failed2;
	printk(KERN_ERR "---> hwicap init success\n");
	return retval;
...
static void __exit hwicap_module_cleanup(void)
{
	...
	printk(KERN_ERR "hwicap_mocule_cleanup called\n");
