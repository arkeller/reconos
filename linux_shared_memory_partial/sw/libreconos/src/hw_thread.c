/***************************************************************************
 * hw_thread.c: ReconOS hardware thread implementation file
 *              Linux version
 *
 * Contains function definitions.
 *
 * The eCos version is called hw_thread.cxx.
 *
...
void *reconos_delegateThread (void *data) {
	...
	pgd = getpgd();
	if(pgd){
		set_os2task(&response, OSIF_CMD_MMU_SETPGD, pgd);
		if (write(fd, &response, sizeof(response)) != sizeof(response)) {
			perror("error while writing data to OSIF");
		}
	}
...
int rthread_create( rthread_t *thread,
	...
	// tlb not set -> use default tlb
	if(attr->tlb == NULL){
		attr->hwt.tlb = &default_tlb;
		tlb_init(attr->hwt.tlb);
	} else {
		attr->hwt.tlb = attr->tlb;
	}

	attr->tlb = attr->hwt.tlb;
...
int rthread_attr_setsharedmemory(rthread_attr_t *attr, char * writebuffer, char * readbuffer) {
	if (writebuffer == NULL || readbuffer == NULL ) {
         	...
         	attr->hwt.writebuffer = writebuffer;
         	attr->hwt.readbuffer = readbuffer;
         	attr->hwt.writeptr = writebuffer;
         	attr->hwt.readptr = readbuffer;
     	}
     	return 0;
}
